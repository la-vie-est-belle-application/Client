...
describe("회원가입 스키마 유효성 검사", () => {
  ...
  describe("아이디 유효성 검사", () => {
    ...
    it("아이디가 비어있으면 에러를 반환한다", () => {
      ...
      if (!result.success) {
        expect(result.error.errors[0].message).toBe("아이디를 입력해주세요");
      }
    });

    it("아이디가 6글자 미만이면 에러를 반환한다", () => {
      const result = userSchema.safeParse({
        ...validUserData,
        userId: "test",
      });
      ...
      if (!result.success) {
        expect(result.error.errors[0].message).toBe(
          "아이디를 6글자, 18글자 이내로 입력해주세요.",
        );
      }
    });

    it("아이디가 18글자를 초과하면 에러를 반환한다", () => {
      const result = userSchema.safeParse({
        ...validUserData,
        userId: "testuser11231321313213",
      });
    ...
    it("아이디가 영문으로 시작하지 않으면 에러를 반환한다", () => {
      const result = userSchema.safeParse({
        ...validUserData,
        userId: "123usr",
      });
      ...
      if (!result.success) {
        expect(result.error.errors[0].message).toBe(
          "아이디는 영문으로 시작하고 영문과 숫자만 사용할 수 있습니다",
        );
      }
    });

    it("아이디에 특수문자가 포함되면 에러를 반환한다", () => {
      const result = userSchema.safeParse({
        ...validUserData,
        userId: "test@1",
      });
  ...
  describe("비밀번호 유효성 검사", () => {
    ...
    it("비밀번호가 8자 미만이면 에러를 반환한다", () => {
      ...
      if (!result.success) {
        expect(result.error.errors[0].message).toBe(
          "문자, 숫자, 특수문자를 사용하여 8글자, 20글자 이내로 입력해주세요.",
        );
      }
    });
